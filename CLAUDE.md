...
- **~174 KB gzip** bundled (~852 KB raw across `dist/index.js` + engine chunks —
  ...
  `src/` costs only its own subgraph). TSDoc documented, 6521 engine tests +
...
docs/             # GitHub Pages site + demos (85 demos in demos/, tiered physics /
...
config. The `templates/` tree is git-tracked. A `create-nape-game`
scaffolder CLI once lived under `packages/` but was removed (AI-first
onboarding made it redundant) — recover from git history if ever needed.
...
3. `npm test` — all tests must pass (6521 + 77)
